...
void blockingDelay(int milliseconds){
  for(int i=0;i<1000*milliseconds;i++){
    _DELAY_1_US
  }
}


void blockingDelay2(int milliseconds){
  for(int i=0;i<10*milliseconds;i++){
...
#endif  // AUTO_BED_LEVELING_FEATURE
{
  ...
  #else
    float delta_mm[4];
    delta_mm[X_AXIS] = dx * steps_to_mm[X_AXIS]; //steps_to_mm[X|Y|Z] = 0.01
    delta_mm[Y_AXIS] = dy * steps_to_mm[Y_AXIS];
    delta_mm[Z_AXIS] = dz * steps_to_mm[Z_AXIS];
  #endif
...
#endif  // AUTO_BED_LEVELING_FEATURE
{
  ...
  switch(OPMODE){
    ...
    case USE_E:
      ...
      break;
    case USE_F_E:
      snw = ALPHA * fr_mm_s * de;
      break;
    case USE_F_E_DIV:
      snw = ALPHA * fr_mm_s * de / sqrt(sq(dx)+sq(dy));
      break;
  }
  ...
  if(snw<0){
   ...
   /* work in progress */
   /*
   if(de!=0 && screw_stopped){
      //open pin
      Serial3.print("SNW,");
      Serial3.println(snw,0);
      screw_stopped = false;
    }
    else if (de!=0 && fabs(previous_snw-snw) > 1){
      Serial3.print("SNW,");
      Serial3.println(snw,0);
      freeze(FREEZE_TIME);
   } else if(de==0 && previous_de!=0 && !screw_stopped){
    //close pin
    freeze(FREEZE_TIME);
    Serial3.print("SNW,");
    Serial3.println(0);
